...
class MoLEncoder(nn.Module):
    ...
    def forward(self, idx, mask):
        ...
        token_embeddings = F.pad(mask_embeddings, pad=(0, 0, 0, self.config['max_len'] - mask_embeddings.shape[1]),
                                 value=0)
...
class Smi_ted(nn.Module):
    ...
    def load_checkpoint(self, ckpt_path):
        ...
        self.net = Net(self.n_embd, n_output=self.config['n_output'] if 'n_output' in self.config else 1,
                       dropout=self.config['d_dropout'])
